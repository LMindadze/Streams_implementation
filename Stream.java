 ...
 */
public interface Stream<T> extends StreamIterator {
...
//     * @throws CheckedStreamException             falls der Stream an dem Punkt ein
    ...
    static <T> Stream<T> of(java.util.stream.Stream<T> javaStream) {
        return new Stream<T>() {
            ...
            public boolean hasNext() {
                return javaStream.iterator().hasNext();
            }

            @Override
            public StreamElement<T> next() {
                return (StreamElement<T>) javaStream.iterator().next();
            }

            @Override
            public OptionalLong getSize() {
                return OptionalLong.of(javaStream.count());
            }

            @Override
            public <R> Stream<R> map(Function<? super T, ? extends R> mapper) {
                return Stream.of(javaStream.map(mapper));
            }

            @Override
            public Stream<T> filter(Predicate<? super T> filter) {
                return Stream.of(javaStream.filter(filter));
            }

            @Override
            public <R> Stream<R> mapChecked(ThrowingFunction<? super T, ? extends R> mapper) {
                return Stream.of(javaStream.filter(x -> x instanceof Exception && !(x instanceof RuntimeException)).map((Function<? super T, ? extends R>) mapper));
            }

            @Override
            public Stream<T> filterChecked(ThrowingPredicate<? super T> filter) {
                return Stream.of(javaStream.filter(x -> x instanceof Exception && !(x instanceof RuntimeException)).filter((Predicate<? super T>) filter));
            }

            @Override
            public Stream<T> distinct() {
                return Stream.of(javaStream.distinct());
            }
            ...
            public Optional<T> reduce(BinaryOperator<T> accumulator) {
                return javaStream.reduce(accumulator);
            }

            @Override
            public Collection<T> toCollection(Supplier<? extends Collection<T>> collectionGenerator) {
                return javaStream.collect(Collectors.toCollection(collectionGenerator));
            }

            @Override
            public Stream<T> onErrorMap(Function<? super List<Exception>, ? extends T> errorMapper) {
                return null; //there are no instructions for these functions (impossible to deduce only from the name)
            }
    ...
    static <T> Stream<T> of(Collection<T> col) {
        return new Stream<T>() {
            @Override
            public boolean hasNext() {
                return col.stream().iterator().hasNext();
            }

            @Override
            public StreamElement<T> next() {
                return (StreamElement<T>) col.stream().iterator().next();
            }

            @Override
            public OptionalLong getSize() {
                return OptionalLong.of(col.size());
            }
            @Override
            public <R> Stream<R> map(Function<? super T, ? extends R> mapper) {
                return Stream.of(col.stream().map(mapper));
            }

            @Override
            public Stream<T> filter(Predicate<? super T> filter) {
                return Stream.of(col.stream().filter(filter));
            }

            @Override
            public <R> Stream<R> mapChecked(ThrowingFunction<? super T, ? extends R> mapper) {
                return Stream.of(col.stream().filter(x -> x instanceof Exception && !(x instanceof RuntimeException)).map((Function<? super T, ? extends R>) mapper));
            }

            @Override
            public Stream<T> filterChecked(ThrowingPredicate<? super T> filter) {
                return Stream.of(col.stream().filter(x -> x instanceof Exception && !(x instanceof RuntimeException)).filter((Predicate<? super T>) filter));
            }

            @Override
            public Stream<T> distinct() {
                return Stream.of(col.stream().distinct());
            }
            ...
            @Override
            public Optional<T> findFirst() {
                return col.stream().findFirst();
            }

            @Override
            public Optional<T> reduce(BinaryOperator<T> accumulator) {
                return col.stream().reduce(accumulator);
            }

            @Override
            public Collection<T> toCollection(Supplier<? extends Collection<T>> collectionGenerator) {
                return col.stream().collect(Collectors.toCollection(collectionGenerator));
            }
    ...
    static <T> Stream<T> of(T... elements) {
        return new Stream<T>() {
            @Override
            public boolean hasNext() {
                return Arrays.stream(elements).iterator().hasNext();
            }

            @Override
            public StreamElement<T> next() {
                return (StreamElement<T>) Arrays.stream(elements).iterator().next();
            }

            @Override
            public OptionalLong getSize() {
                return OptionalLong.of(elements.length);
            }
            @Override
            public <R> Stream<R> map(Function<? super T, ? extends R> mapper) {
                return Stream.of(Arrays.stream(elements).map(mapper));
            }

            @Override
            public Stream<T> filter(Predicate<? super T> filter) {
                return  Stream.of(Arrays.stream(elements).filter(filter));
            }

            @Override
            public <R> Stream<R> mapChecked(ThrowingFunction<? super T, ? extends R> mapper) {
                return Stream.of(Arrays.stream(elements).filter(x -> x instanceof Exception && !(x instanceof RuntimeException)).map((Function<? super T, ? extends R>) mapper));
            }

            @Override
            public Stream<T> filterChecked(ThrowingPredicate<? super T> filter) {
                return Stream.of(Arrays.stream(elements).filter(x -> x instanceof Exception && !(x instanceof RuntimeException)).filter((Predicate<? super T>) filter));
            }

            @Override
            public Stream<T> distinct() {
                return Stream.of(Arrays.stream(elements).distinct());
            }
            ...
            @Override
            public Optional<T> reduce(BinaryOperator<T> accumulator) {
                return Arrays.stream(elements).reduce(accumulator);
            }

            @Override
            public Collection<T> toCollection(Supplier<? extends Collection<T>> collectionGenerator) {
                return Arrays.stream(elements).collect(Collectors.toCollection(collectionGenerator));
            }
